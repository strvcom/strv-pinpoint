...
(() => {
  ...
  style.textContent = '[data-pinpoint] .ff-icon{background:transparent;border:0;color:#cfd3dc;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;transition:color .15s ease,background .15s ease,transform .15s ease}[data-pinpoint] .ff-icon:hover{color:#fff;transform:scale(1.12)}[data-pinpoint] .ff-icon.ff-active{color:#fff;background:#2962ff}[data-pinpoint] .ff-pill{transform-origin:right center;transition:transform .18s ease,opacity .18s ease}[data-pinpoint] .ff-card{transform-origin:top left;transition:transform .15s ease,opacity .15s ease}[data-pinpoint] .ff-badge{transition:transform .16s ease,opacity .16s ease}';
  ...
  function createNodes(it) {
    ...
    var badge = document.createElement('div'); badge.setAttribute('data-pinpoint', '1'); badge.className = 'ff-badge'; badge.style.cssText = 'position:absolute;pointer-events:auto;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;width:18px;height:18px;border-radius:9px;background:' + color + ';color:#06210f;font:bold 11px system-ui;left:0;top:0';
  ...
  function buildCard(it) {
    ...
    var color = it.kind === 'screenshot' ? '#a855f7' : '#22c55e';
    var card = document.createElement('div'); card.setAttribute('data-pinpoint', '1'); card.className = 'ff-card'; card.style.cssText = 'position:absolute;pointer-events:auto;width:208px;background:#1b1b1b;border:1px solid #2a6;border-radius:8px;padding:8px;box-shadow:0 6px 20px rgba(0,0,0,.4);z-index:' + (Z + 6) + ';left:0;top:0;opacity:0;transform:scale(.85)';
    ...
    var del = document.createElement('button'); del.className = 'ff-icon'; del.innerHTML = ICON.trash; del.title = 'delete annotation'; del.style.cssText = 'width:24px;height:24px;border-radius:6px'; del.onclick = function (e) { e.stopPropagation(); if (card.querySelector('.ff-confirm')) return; state.confirming = true; var ov = document.createElement('div'); ov.className = 'ff-confirm'; ov.setAttribute('data-pinpoint', '1'); ov.style.cssText = 'position:absolute;inset:0;background:rgba(18,18,18,.97);border-radius:8px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;padding:10px;z-index:3'; var msg = document.createElement('div'); msg.textContent = 'Delete this annotation?'; msg.style.cssText = 'font:600 12px system-ui;color:#fff'; ov.appendChild(msg); ov.appendChild(confirmRow('Delete', function () { state.confirming = false; deleteAnnotation(it); }, function () { state.confirming = false; if (ov.parentNode) ov.parentNode.removeChild(ov); try { ta.focus({ preventScroll: true }); } catch (_) {} })); card.appendChild(ov); };
