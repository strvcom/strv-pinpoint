 ...
 * when the mouse is near. Toolbar + card buttons share .pp-icon styling (bright-gray → white,
 * blue when active). Overlay elements carry data-pinpoint so the bridge can hide them while
 * capturing screenshots. State on window.__pinpointAnnotations = {batchId, ready, items};
 * window.__pinpointSelection = most-recent; window.__pinpointRegion = last screenshot rect.
...
(() => {
  function install() {
  if (window.__pinpointOverlayInstalled || !document.body) return;
  ...
  var fab = document.createElement('div'); fab.setAttribute('data-pinpoint', '1'); fab.style.cssText = 'position:fixed;z-index:' + (Z + 3) + ';display:flex;align-items:center;gap:6px;font:12px system-ui;color:#fff'; document.documentElement.appendChild(fab);
  var pill = document.createElement('div'); pill.className = 'pp-pill'; pill.style.cssText = 'display:none;align-items:center;gap:4px;background:#1b1b1b;border:1px solid #444;border-radius:22px;padding:5px 8px';
  var handle = document.createElement('span'); handle.className = 'pp-icon'; handle.innerHTML = ICON.grip; handle.style.cssText = 'cursor:grab;user-select:none'; pill.appendChild(handle);
  function tbtn(html, title, fn) { var b = document.createElement('button'); b.className = 'pp-icon'; b.innerHTML = html; b.title = title; b.style.cssText = 'min-width:30px;height:26px;padding:0 8px;border-radius:14px;font:12px system-ui'; b.onclick = function (e) { e.stopPropagation(); fn(); }; return b; }
  ...
  var orb = document.createElement('button'); orb.textContent = '✦'; orb.title = 'pinpoint — click to open; drag to move'; orb.style.cssText = 'width:40px;height:40px;border-radius:20px;border:0;background:#2962ff;color:#fff;font:18px system-ui;cursor:pointer;box-shadow:0 6px 20px rgba(0,0,0,.5)';
  ...
  function setMode(m) { state.mode = m; hover.style.display = 'none'; if (m !== 'screenshot') marquee.style.display = 'none'; document.body.style.cursor = m ? 'crosshair' : ''; bPick.classList.toggle('pp-active', m === 'pick'); bShot.classList.toggle('pp-active', m === 'screenshot'); }
  ...
  function clearAnnotations() {
    ...
    var p = document.createElement('div'); p.setAttribute('data-pinpoint', '1');
  ...
  function createNodes(it) {
    ...
    var bx = document.createElement('div'); bx.setAttribute('data-pinpoint', '1'); bx.style.cssText = 'position:absolute;border:2px solid ' + color + ';background:rgba(34,197,94,.10);left:0;top:0;width:0;height:0'; marks.appendChild(bx);
    var badge = document.createElement('div'); badge.setAttribute('data-pinpoint', '1'); badge.className = 'pp-badge'; badge.style.cssText = 'position:absolute;pointer-events:auto;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;width:18px;height:18px;border-radius:9px;background:' + color + ';color:#06210f;font:bold 11px system-ui;left:0;top:0';
  ...
  function buildCard(it) {
    ...
    var color = it.kind === 'screenshot' ? '#a855f7' : '#22c55e';
    var card = document.createElement('div'); card.setAttribute('data-pinpoint', '1'); card.className = 'pp-card'; card.style.cssText = 'position:absolute;pointer-events:auto;width:208px;background:#1b1b1b;border:1px solid #2a6;border-radius:8px;padding:8px;box-shadow:0 6px 20px rgba(0,0,0,.4);z-index:' + (Z + 6) + ';left:0;top:0;opacity:0;transform:scale(.85)';
    ...
    var cam = document.createElement('button'); cam.className = 'pp-icon' + (it.wantScreenshot ? ' pp-active' : ''); cam.innerHTML = ICON.camera; cam.title = 'include a screenshot of this'; cam.style.cssText = 'width:24px;height:24px;border-radius:6px'; cam.onclick = function (e) { e.stopPropagation(); it.wantScreenshot = !it.wantScreenshot; cam.classList.toggle('pp-active', it.wantScreenshot); setDirty(); };
    var mini = document.createElement('button'); mini.className = 'pp-icon'; mini.innerHTML = ICON.min; mini.title = 'minimize'; mini.style.cssText = 'width:24px;height:24px;border-radius:6px'; mini.onclick = function (e) { e.stopPropagation(); closeCard(it); };
    var del = document.createElement('button'); del.className = 'pp-icon'; del.innerHTML = ICON.trash; del.title = 'delete annotation'; del.style.cssText = 'width:24px;height:24px;border-radius:6px'; del.onclick = function (e) { e.stopPropagation(); if (card.querySelector('.pp-confirm')) return; state.confirming = true; var ov = document.createElement('div'); ov.className = 'pp-confirm'; ov.setAttribute('data-pinpoint', '1'); ov.style.cssText = 'position:absolute;inset:0;background:rgba(18,18,18,.97);border-radius:8px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;padding:10px;z-index:3'; var msg = document.createElement('div'); msg.textContent = 'Delete this annotation?'; msg.style.cssText = 'font:600 12px system-ui;color:#fff'; ov.appendChild(msg); ov.appendChild(confirmRow('Delete', function () { state.confirming = false; deleteAnnotation(it); }, function () { state.confirming = false; if (ov.parentNode) ov.parentNode.removeChild(ov); try { ta.focus({ preventScroll: true }); } catch (_) {} })); card.appendChild(ov); };
  ...
  async function doSend() { if (!state.items.length) return; state.ready = true; state.batchId++; sync(); bSend.textContent = 'Copied ✓'; bSend.title = 'Copied — paste with Cmd+Shift+V into Claude'; bSend.style.background = '#0a4'; if (window.__pinpointLink) { try { state.lastPromptId = await window.__pinpointLink.send(serialize().items); } catch (_) {} } }
